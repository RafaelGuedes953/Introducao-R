...
# Para ser usado apenas na primeira vez, instalação do pacote
...
# Para ler uma planilha para dentro do R que esteja salva com a extensão ".csv", 
...
databaseFev <- read_csv2("dados/voos_de_fevereiro.csv")
...
# Equivalentes a uma tabela do SQL ou uma planilha do Excel.

# A principal característica de um data.frame é possuir linhas e colunas! 

# Explorando dados da base (funções úteis)-------------------------------------

database # somente o nome da base: visualizar no console

View(database) # a função View() abre uma nova aba onde é possível ver a base.
...
which(names(tabela) == "o que quero_saber")
