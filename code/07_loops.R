...
# o vetor VARIAVEL, pode ter qualquer nome também! você só precisa se lembrar de fazer menção a ela
# na ação
...
for (a in 1:50) {
    ...
    print(paste("Esse aqui é o número", a))
...
for(i in 7:31){
    ...
    print("Iniciando a ação")
    print(paste("Calculando o quadrado de",i))
    print(paste("O quadrado de",i,"vale",quadrado))
}
...
for(arquivo in arquivos){
    ...
    # browser() # Função que passa o controle da execução do loop para o usuário
...
for(arquivo in arquivos){
    ...
    text_out <- str_glue("O maior atraso no arquivo {arquivo}: {maior_atraso}\nO maior adiantamento: {mais_adiantado}")
